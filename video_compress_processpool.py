...
import os, sys, math, json, tempfile, shutil, subprocess, pathlib, statistics
...
CSV_FALLBACK_PATH = "video_5s_results.csv"

FFMPEG_THREADS = "1"
VMAF_SUBSAMPLE = 5
VMAF_THREADS   = 1
# Parallel processing: number of jobs (None = all cores, or specify a number)
N_JOBS = None  # None uses all available CPU cores
...
# ----------------------------
# One video wrapper (with error handling for parallel execution)
# ----------------------------
def process_one(src: str) -> Dict:
    try:
        ...
        if best is None:
            result = {"file": os.path.basename(src), "duration_sec": dur,
        ...
        vmaf_str = "NA" if (result.get("VMAF") is None) else f"{result['VMAF']:.2f}"
        c_str    = "NA" if (result.get("C")    is None) else f"{result['C']:.4f}"
        s_str    = "NA" if (result.get("S")    is None) else f"{result['S']:.3f}"
        crf_str  = str(result.get("CRF","NA"))
        print(f"{result['file']} | VMAF={vmaf_str} | C={c_str} | S={s_str} | CRF={crf_str} | If_Downscaled={result.get('If_Downscaled','NO')} | {result.get('note','')}")
        ...
        return {"file": os.path.basename(src), "duration_sec": 0.0,
                "VMAF": None, "C": None, "S": 0.0, "CRF": None,
                "If_Downscaled": "NO", "lap_var": None, "note": f"error: {str(e)}"}
...
def main():
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(future_to_video):
            ...
            try:
                ...
                print(f"[Progress: {completed_count}/{len(videos)}] Completed: {result.get('file', 'unknown')}")
            except Exception as e:
    ...
    # Sort results by original video order (maintain order of input videos)
    video_to_result = {r["file"]: r for r in all_results}
    sorted_results = []
    for v in videos:
        video_name = os.path.basename(v)
        if video_name in video_to_result:
            sorted_results.append(video_to_result[video_name])
        else:
            # Fallback: create empty result if somehow missing
