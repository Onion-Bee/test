...
VMAF_SUBSAMPLE = 5
VMAF_THREADS   = 1
# Parallel processing: number of jobs (-1 = all cores, or specify a number)
N_JOBS = -1
...
def main():
    ...
    print(f"Processing {len(videos)} videos with {N_JOBS if N_JOBS > 0 else 'all available'} parallel jobs...\n")
    
    # Process videos in parallel
    all_results = Parallel(n_jobs=N_JOBS, verbose=10)(
        delayed(process_one)(v) for v in videos
    )
